...
public class AdviceScheduler {
    ...
    public void tick() {
        ...
        try(var connection=dataSource.getConnection()) {
            ...
            positions.job("portfolio-advice","FAILED","诊断与建议后台检查失败，原始报告与核验结果保留。",clock.instant(),true);
        }
    }
    public void run() {
        positions.job("portfolio-advice","RUNNING","正在保存今日诊断与建议并检查到期记录。",clock.instant(),false);
        ...
        positions.job("portfolio-advice",failed==0 ? "SUCCEEDED" : "PARTIAL",failed==0 ? "每日诊断、建议与到期记录检查完成。" : "部分记录待重试，已保存报告保留。",clock.instant(),true);
        LOGGER.info("AdviceScheduler.run   >>>   checked={}, failures={}",saved,failed);
    }
    private static final DiagnosisTypes.Facts FAILED_FACTS=new DiagnosisTypes.Facts(null,null,null,null,List.of());
