...
/**
 * WatchlistService 的 JDBC 实现，适用于明确的 M1 单用户部署模式。
 *
...
public class JdbcWatchlistService implements WatchlistService {
    ...
    public WatchlistItemResponse addCurrentUserItem(String fundCode) {
        ...
        int inserted = jdbcClient.sql("""
                ...
                .update();
        WatchlistItemResponse item = findRequiredItem(fundCode);
        writeAudit(inserted == 1 ? "WATCHLIST_ADDED" : "WATCHLIST_ADD_IDEMPOTENT", fundCode);
        LOGGER.info("JdbcWatchlistService.addCurrentUserItem   >>> fundCode={}, inserted={}", fundCode, inserted == 1);
        return item;
    }
    ...
    public void removeCurrentUserItem(String fundCode) {
        int deleted = jdbcClient.sql("""
                ...
                .param("fundCode", fundCode)
                .update();
        writeAudit(deleted == 1 ? "WATCHLIST_REMOVED" : "WATCHLIST_REMOVE_IDEMPOTENT", fundCode);
        LOGGER.info("JdbcWatchlistService.removeCurrentUserItem   >>> fundCode={}, deleted={}", fundCode, deleted == 1);
    }

    /** 查询刚插入或既有的关注记录；理论上缺失表示数据库写入出现不一致。 */
    private WatchlistItemResponse findRequiredItem(String fundCode) {
        return jdbcClient.sql("""
                        SELECT fund_code, created_at
                        FROM watchlist_item
                        WHERE user_id = :userId AND fund_code = :fundCode
    ...
    private void writeAudit(String action, String fundCode) {
        jdbcClient.sql("""
